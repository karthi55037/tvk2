...
export async function GET(req: NextRequest, { params }: { params: { itemId: string } }) {
  return handle(async () => {
    ...
    return Response.json({ messages: await listMessages(user, { threadType: 'ASSIGNMENT', threadId: params.itemId }, after) });
  });
}
...
export async function POST(req: NextRequest, { params }: { params: { itemId: string } }) {
  return handle(async () => {
    ...
    return Response.json({ message: await postMessage(user, { threadType: 'ASSIGNMENT', threadId: params.itemId }, input) });
  });
}
